...
void ForwardEliminationOptimized(double *matrix, double *rhs, uint_fast32_t rows, uint_fast32_t columns){
    for(uint_fast32_t row = 0; row < rows; row++){
        ...
        for (uint_fast32_t lower_rows=row+1; lower_rows<rows; lower_rows++){
             ...
             *  x7  x8  x9
             */
            uint_fast32_t d = IDX(lower_rows, row);
            double elimination_factor = matrix[d]/diag_elem;
            d++;
            uint_fast32_t b = IDX(row,row+1);
            for (uint_fast32_t column=row+1; column<columns; column++){
                 ...
                 *
                 */
                matrix[d++] -= elimination_factor*matrix[b++];
            }
            rhs[lower_rows] -= elimination_factor*rhs[row];
...
void BackwardSubstitutionOptimized(double *matrix, double *rhs, double*solution, uint_fast32_t rows, uint_fast32_t columns){
    for(int_fast32_t row=rows-1; row>=0; row--){
         ...
         * y1 -= x2 * y2 - x3 * y3
         */
        uint_fast32_t a = IDX(row, row+1);
        for (uint_fast32_t column=row+1; column<columns; column++){
            solution[row] -= matrix[a++]*solution[column];
        }
...
void Serial::Solve(double *matrix, double *rhs, double *solution,
    ...
    Serial::BackwardSubstitutionOptimized(matrix, rhs, solution, rows, columns);
}
